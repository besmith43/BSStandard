...
namespace Edit.cmd
{
    public class Options
    {
        ...
        public Options()
        {
            helpFlag = false;
            versionFlag = false;
            versionText = $"Edit { Assembly.GetEntryAssembly().GetCustomAttribute<AssemblyFileVersionAttribute>().Version }";
            verbose = false;
            StringBuilder helpTextBuilder = new StringBuilder();
            helpTextBuilder.AppendLine($"{ versionText }");
            helpTextBuilder.AppendLine("Usage:  Edit [OPTION] <path to file> ");
