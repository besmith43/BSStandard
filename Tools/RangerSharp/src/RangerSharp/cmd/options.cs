...
namespace RangerSharp.cmd
{
    public class Options
    {
        ...
        public Options()
        {
            helpFlag = false;
            versionFlag = false;
            versionText = $"RangerSharp { Assembly.GetEntryAssembly().GetCustomAttribute<AssemblyFileVersionAttribute>().Version }";
            verbose = false;
            StringBuilder helpTextBuilder = new StringBuilder();
            helpTextBuilder.AppendLine($"{ versionText }");
            helpTextBuilder.AppendLine("Usage:  RangerSharp [OPTION]  ");
