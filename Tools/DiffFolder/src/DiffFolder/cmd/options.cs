...
namespace DiffFolder.cmd
{
    public class Options
    {
        ...
        public Options()
        {
            helpFlag = false;
            versionFlag = false;
            versionText = $"DiffFolder { Assembly.GetEntryAssembly().GetCustomAttribute<AssemblyFileVersionAttribute>().Version }";
            verbose = false;
            StringBuilder helpTextBuilder = new StringBuilder();
            helpTextBuilder.AppendLine($"{ versionText }");
            helpTextBuilder.AppendLine("Usage:  DiffFolder [OPTION] <folder1> <folder2> ");
